...
//
    ...
    raibo_arm->getBodyOrientation(armIndices_[0], ori_);
...
//
    ...
    std::cout << "givne velocity : " << ee_Vel_w_.e() << std::endl;

    std::cout << "analytic velocity : " << ee_Jaco_body_*gv_ << std::endl;
...
//
    ...
    GenForce_ << ee_Jaco_body_.transpose() * (ee_Jaco_body_*raibo_arm->getInverseMassMatrix().e()*ee_Jaco_body_.transpose()).inverse() * (posPgain_*baseRot_.e().transpose()*(pos_residual_ - (point_.e() - gc_.head(3))) - posDgain_*baseRot_.e().transpose()*ee_Vel_w_.e()) +
        ee_Jaco_rot_.transpose() * (ee_Jaco_rot_ * raibo_arm->getInverseMassMatrix().e() * ee_Jaco_rot_.transpose()).inverse() * (- oriDgain_*baseRot_.e().transpose()*ee_Avel_w_.e()) +
        raibo_arm->getNonlinearities(world.getGravity()).e();
    GenForce_.head(18).setZero();
    raibo_arm->setPdTarget(jointNominalConfig, jointVelocityTarget);
    raibo_arm->setGeneralizedForce(GenForce_);

    std::cout << "poistion : " << std::endl << baseRot_.e().transpose()*(pos_residual_ - (point_.e() - gc_.head(3))) << std::endl;

    std::cout << "Orientation : " << std::endl << ori_.e() << std::endl;
...
//    raibo_arm->setGeneralizedForce(GenForce_);
    ...
    for (auto &contact: raibo_arm->getContacts())
      std::cout << "conatcts : " << std::endl << ((contact.getPairObjectIndex() == 0) || (contact.getPairObjectIndex() == 1)) << std::endl;
