...
//  auto raibo_arm = world.addArticulatedSystem(binaryPath.getDirectory() + "\\rsc\\raibo_arm\\urdf\\raibo_arm.urdf");
  ...
  Eigen::Matrix3d ee_M_app_body_, ee_M_app_rot_;
...
//  std::cout << "sparse jacobian rot : " << ee_Jaco_rot_s_.v << std::endl;
//  for (auto i: ee_Jaco_rot_s_.idx) {
//    std::cout << "sparse jacobian rot idx : " << i << std::endl;
//  }


//  raibo_arm->getTimeDerivativeOfSparseJacobian(raibo_arm->getFrameIdxByName("ee_link"), raisim::ArticulatedSystem::PARENT_FRAME, point_, ee_Jaco_body_dot_s_);
//  raibo_arm->getTimeDerivativeOfSparseRotationalJacobian(raibo_arm->getFrameIdxByName("ee_link"), ee_Jaco_rot_dot_s_);
//
//  std::cout << "sparse jacobian : " << ee_Jaco_body_dot_s_.e() << std::endl;
...
//        raibo_arm->getTimeDerivativeOfSparseJacobian(footIndices_[index], raisim::ArticulatedSystem::PARENT_FRAME, point_, dJc_tmp_s_);
//
//        int j = 0;
...
//
    ...
    ee_M_app_body_ << (ee_Jaco_body_ * raibo_arm->getInverseMassMatrix().e() * ee_Jaco_body_.transpose());
//    ee_M_app_rot_ << (ee_Jaco_rot_ * raibo_arm->getInverseMassMatrix().e() * ee_Jaco_rot_.transpose());
//
//    ee_acc_ << (posPgain_ * pos_residual_ - posDgain_ * ee_Vel_);
//    ee_Aacc_ <<
//             (oriPgain_ * rot_residual_ - oriDgain_ * ee_Avel_);
//
//    Q_(Eigen::seqN(0, Jc_batch_.size()*3 + 6), Eigen::all) << J_;
//    Q_(Eigen::seqN(Jc_batch_.size()*3 + 6, Jc_batch_.size()*3 + 6), Eigen::all) << dJ_;
//
//    R_(Eigen::seqN(Jc_batch_.size()*3 + 6, Jc_batch_.size()*3 + 6), Eigen::all) << J_;
//    V_ << Eigen::VectorXd::Zero(Jc_batch_.size()*3), ee_Vel_, ee_Avel_, Eigen::VectorXd::Zero(Jc_batch_.size()*3), ee_acc_, ee_Aacc_;
//
...
//    raibo_arm->setGeneralizedForce(S_arm_.transpose()*raibo_arm->getNonlinearities(world.getGravity()).e().tail(6) + ee_Jaco_body_.transpose()*ee_M_app_body_.inverse()*(-10*ee_Jaco_body_*gv_));
    ...
    GenForce_ << ee_Jaco_body_.transpose() * ((ee_Jaco_body_ * raibo_arm->getInverseMassMatrix().e() * ee_Jaco_body_.transpose()).inverse() *(
        (posPgain_ * pos_residual_ - posDgain_ * ee_Vel_w_.e())
    ...
    std::cout << "GenForce : " << std::endl << GenForce_ << std::endl;
