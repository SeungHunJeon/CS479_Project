...
//    Q_.setZero((Jc_batch_.size()*3 + 3), raibo_arm->getDOF());
//    R_.setZero((Jc_batch_.size()*3 + 3), raibo_arm->getDOF());
    V_.setZero((Jc_batch_.size()*3 + 3));
    J_.setZero((Jc_batch_.size()*3 + 3), raibo_arm->getDOF());
    dJ_.setZero((Jc_batch_.size()*3 + 3), raibo_arm->getDOF());
...
//    std::cout << "R : " << std::endl << R_ << std::endl;
    ...
    V_ << Eigen::VectorXd::Zero(Jc_batch_.size()*3 + 3);

    std::cout << "V : " << std::endl << V_ << std::endl;

    N_ = Eigen::MatrixXd::Identity(raibo_arm->getDOF(), raibo_arm->getDOF()) - J_.completeOrthogonalDecomposition().pseudoInverse()*J_;
...
//    std::cout << "P : " << std::endl << P_ << std::endl;
    ...
    Out_  = K_.completeOrthogonalDecomposition().pseudoInverse() *
        (raibo_arm->getMassMatrix().e() * J_.completeOrthogonalDecomposition().pseudoInverse() * V_ +
            P_*(raibo_arm->getNonlinearities(world.getGravity()).e() - S_leg_.transpose() * raibo_arm->getGeneralizedForce().e()(Eigen::seqN(6,12))
             - ee_Jaco_body_.transpose() * F_a_) +
            (Jc_.transpose() * (Jc_ * raibo_arm->getInverseMassMatrix().e() * Jc_.transpose()).inverse() * dJc_ - raibo_arm->getMassMatrix().e() * J_.completeOrthogonalDecomposition().pseudoInverse() * dJ_) * gv_);
