...
class RolloutStorage:
    def __init__(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, obs_shape, actions_shape, device):
        ...
        self.prob = np.zeros([num_transitions_per_env, num_envs, *actions_shape], dtype=np.float32)
        ...
        self.prob_tc = torch.from_numpy(self.prob).to(self.device)
    ...
    def add_transitions(self, actor_obs, critic_obs, obs, actions, prob, rewards, dones, actions_log_prob):
    ...
    def mini_batch_generator_shuffle(self, num_mini_batches):
        ...
        for indices in BatchSampler(SubsetRandomSampler(range(batch_size)), mini_batch_size, drop_last=True):
            ...
            prob_batch = self.prob_tc.view(-1, self.prob_tc.size(-1))[indices]
            ...
            yield actor_obs_batch, critic_obs_batch, obs_batch, actions_batch, prob_batch, values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch
    ...
    def mini_batch_generator_inorder(self, num_mini_batches):
        ...
        for batch_id in range(num_mini_batches):
            yield self.obs_tc[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                ...
                self.prob_tc.view(-1, self.prob_tc.size(-1))[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
