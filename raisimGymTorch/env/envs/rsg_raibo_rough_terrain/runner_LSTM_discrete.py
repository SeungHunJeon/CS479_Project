...
from raisimGymTorch.env.bin.rsg_raibo_rough_terrain import DiscreteSampler
...
import raisimGymTorch.algo.ppo.ppo_LSTM_discrete as PPO
...
actor = ppo_module.Actor(ppo_module.MLP(cfg['architecture']['encoding']['policy_net'],
                                        nn.LeakyReLU,
                                        hidden_dim,
                                        prob_dim,
                                        actor=True,
                                        discrete=True),
                         ppo_module.DiscreteDistribution(prob_dim,
                                                         env.num_envs,
                                                         DiscreteSampler(prob_dim),
                                                         cfg['seed']),
                         device)
critic = ppo_module.Critic(ppo_module.MLP(cfg['architecture']['encoding']['value_net'],
                                          nn.LeakyReLU,
                                          hidden_dim,
                                          1,
                                          actor=False),
