...
from .math_utils import rot_matrix_to_vec, vec_to_rot_matrix
...
class Planner:
    def __init__(self, start_state, end_state, cfg, density_fn):
        ...
        self.initial_commands = torch.cat([cfg['command'], cfg['command']], dim=0).requires_grad_(True)
    ...
    def a_star_init(self):
        ...
        x_linspace = torch.linspace(-2,0, side)
        y_linspace = torch.linspace(0,2, side)
        z_linspace = torch.linspace(0,2, side)

        coods = torch.stack( torch.meshgrid( x_linspace, y_linspace, z_linspace ), dim=-1)

        kernel_size = 5 # 100/5 = 20. scene size of 2 gives a box size of 2/20 = 0.1 = drone size
        #커널사이즈=>
        #그리드를 100개로 쪼개고 커널사이즈로 그 그리드를 5개짜리 맥스풀로 줄이면 20*20*20맵이나옴
        #맵 사이즈가 2니까 1개 그리드는 0.1m 고 1개 그리드가 로봇 사이즈와 일치해야
        ...
        # occupied = maxpool(output[None,None,...])[0,0,...] < 0.3
        # Occupied map 3d plot
        # fig = plt.figure()
        # ax = fig.add_subplot(111, projection='3d')
        # x, y, z = np.meshgrid(np.arange(occupied.shape[0]), np.arange(occupied.shape[1]), np.arange(occupied.shape[2]))
        # ax.scatter(x, y, z, c=occupied.detach().cpu().numpy().flatten(), cmap='viridis')
        # plt.show()
    ...
    def params(self):
        return [self.initial_commands, self.states]

    def calc_everything(self):

        start_pos   = self.start_state[None, 0:3]
        start_v     = torch.zeros((1,3)).to(device)
        start_v[0,:2] = self.initial_commands[0, :2]
        start_R     = self.start_state[3:].reshape((1, 3, 3))
        start_omega = torch.Tensor([0,0,self.initial_commands[0,-1]]).unsqueeze(0).to(device)

        end_pos   = self.end_state[None, 0:3]
        end_v     = torch.zeros((1,3)).to(device)
        end_R     = self.end_state[3:].reshape((1, 3, 3))
        end_omega = torch.zeros((1,3)).to(device)
        ...
        for i in range(self.K):
            rot = next_rotation(start_R, start_omega, self.dt/self.K*i)
            next_pos += (rot @ start_v.unsqueeze(-1)).squeeze(-1) * self.dt / self.K

        next_R = rot
        next_vel = torch.zeros((1,3))
        next_vel[0,:2] = self.initial_commands[1, :2]
        next_omega = torch.Tensor([0,0,self.initial_commands[1,-1]])

        after_next_pos = next_pos
        for i in range(self.K):
            rot = next_rotation(next_R, next_omega, self.dt/self.K*i)
            after_next_pos += (rot @ next_vel.unsqueeze(-1)).squeeze(-1) * self.dt / self.K
        ...
        current_pos = torch.cat( [start_pos, next_pos, after_next_pos, self.states[1:-1, :3], end_pos], dim=0)
        #
        ...
        rot_matrix = self.states[1:-1, 3:]

        zero_rp_matrix = torch.zeros_like(rot_matrix).expand(-1, 2)

        rot_matrix = torch.cat([zero_rp_matrix, rot_matrix], dim=-1)

        rot_matrix = vec_to_rot_matrix(rot_matrix)

        rot_matrix = torch.cat( [start_R, next_R, after_next_R, rot_matrix, end_R], dim=0)
        ...
        # R_t @ (delta R)
        rot_matrix_mid = next_rotation(rot_matrix[:-1, ...], current_omega / 2, self.dt)
        command_vel = (rot_matrix_mid.swapdims(-1,-2) @ (current_vel[:-1,...].unsqueeze(-1))).squeeze(-1)
        command = torch.cat([command_vel[..., :2], current_omega[..., 2:]], dim=-1)
        command = torch.cat([command, command[-1:, :]], dim=0)
        ...
        return current_pos, current_vel, rot_matrix, current_omega, command
    ...
    def get_actions(self):
        # pos, vel, accel, rot_matrix, omega, angular_accel, actions = self.calc_everything()
        pos, vel, rot_mat, omega, actions = self.calc_everything()
    ...
    def body_to_world(self, points):
        pos, vel, rot_matrix, omega, actions = self.calc_everything()
    ...
    def get_state_cost(self):
        ...
        torques = torch.norm(actions, dim=-1).to(device)
    ...
    def update_state(self, measured_state):
        ...
        self.initial_commands = actions[1:3, :].detach().requires_grad_(True)
