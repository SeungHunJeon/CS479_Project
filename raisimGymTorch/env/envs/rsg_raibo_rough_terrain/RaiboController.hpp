...
class RaiboController {
 public:
  ...
  void updateObservation(bool nosify,
    ...
    obDouble_.segment(26 + 2 * nJoints_ * 3 + 4 * scanConfig_.sum(), 12) = previousAction_ - prevprevAction_;

    /// command
    obDouble_.segment(38 + 2 * nJoints_ * 3 + 4 * scanConfig_.sum(), 3) = command;
  }
  ...
  inline void accumulateRewards(double curriculumFactor, const Eigen::Vector3d &command) {
    ...
    for (size_t i = 0; i < 4; i++)
      if (!footContactState_[i] && prevfootContactState_[i])
  ...
  static constexpr size_t obDim_ = 257;
